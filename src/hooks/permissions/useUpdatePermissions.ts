...
export const useUpdatePermissions = (userId: number) => {
  ...
  const [updatedPermissions, setUpdatedPermissions] = useState<UserPermissions | null>(null); // Stocke les permissions mises à jour
  ...
  const updatePermissions = useCallback(
    async (permissionsToUpdate: Partial<UserPermissions>) => {
      ...
      try {
        ...
        const response = await axios.put(`permissions/update_permissions/${userId}`, permissionsToUpdate, {
        ...
        const errorMessage = err.response?.data?.detail || 'Erreur lors de la mise à jour des permissions';
