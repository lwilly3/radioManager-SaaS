...
export const useRoleTemplates = (): UseRoleTemplatesReturn => {
  ...
  const applyTemplate = useCallback(
    async (userId: number, templateId: string): Promise<boolean> => {
      if (!token) {
        setError("Aucun token d'authentification disponible");
        return false;
      }
  ...
  const createTemplate = useCallback(
    async (
      ...
      try {
        const response = await api.post('permissions/templates', template, {
          headers: { Authorization: `Bearer ${token}` },
        });
        return response.data;
      } catch (err: any) {
        const errorMessage =
          err.response?.data?.detail || 'Erreur lors de la création du modèle';
        setError(errorMessage);
        return null;
      } finally {
        setIsLoading(false);
  ...
  const updateTemplate = useCallback(
    async (template: RoleTemplate): Promise<boolean> => {
      if (!token) {
        setError("Aucun token d'authentification disponible");
        return false;
      }
      ...
      try {
        await api.put(`permissions/templates/${template.id}`, template, {
          headers: { Authorization: `Bearer ${token}` },
        });
        return true;
      } catch (err: any) {
